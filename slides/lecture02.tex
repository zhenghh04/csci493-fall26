...
% ---------------------------------------------------------------- cautions
\begin{frame}{Two cautions before you run it}
  \begin{featbox}{accentgold}{\faGlobe\ Auth can stall on campus Wi-Fi}
    \small The first login is a Globus \emph{browser} flow. If it hangs, try a
    personal hotspot, or use the instructor's pre-run notebook output as a fallback ---
    the \emph{ideas} don't depend on your token working in the first five minutes.
  \end{featbox}
  \vspace{8pt}
  \begin{featbox}{accentblue}{\faCompass\ This is not an HPC lesson}
    \small We are using a supercomputer, but today it's just ``a model behind a URL.''
    Spend your attention on \textbf{what the model does}, not on clusters or queues.
    We never leave the notebook.
  \end{featbox}
  \vspace{8pt}
  \begin{center}\small\color{labnavy}\bfseries
    Goal of the lab: you have \emph{talked to a raw model} and \emph{caught it lying}.\end{center}
\end{frame}
