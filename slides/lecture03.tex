...
% ---------------------------------------------------------------- UI options
\begin{frame}{Prefer a UI? Same config --- three front-ends}
  \begin{columns}[t, totalwidth=\textwidth]
    \begin{column}{0.31\textwidth}
      \begin{featbox}{accentblue}{\faTerminal\ Terminal (TUI)}
        \scriptsize Just \texttt{opencode}. The default --- nothing extra to install.
        \\[3pt]{\color{midgray}For the terminal-comfortable.}
      \end{featbox}
    \end{column}
    \begin{column}{0.035\textwidth}\end{column}
    \begin{column}{0.31\textwidth}
      \begin{featbox}{cRAG}{\faWindowMaximize\ Browser UI}
        \scriptsize \texttt{opencode web} --- starts a local server and \textbf{opens
        opencode in your browser} (\texttt{-{}-port 4096} to fix it).
        \\[3pt]{\color{midgray}Click, don't type. Stays on \texttt{127.0.0.1}.}
      \end{featbox}
    \end{column}
    \begin{column}{0.035\textwidth}\end{column}
    \begin{column}{0.31\textwidth}
      \begin{featbox}{cEval}{\faCode\ VS Code / Cursor}
        \scriptsize Install \textbf{``opencode''} from the Marketplace; open with
        \textbf{Ctrl+Esc}. Shares your open file.
        \\[3pt]{\color{midgray}If you already live in an IDE.}
      \end{featbox}
    \end{column}
  \end{columns}
  \vspace{10pt}
  \begin{calloutbox}[cAgent]
    \small\faInfoCircle\ \textbf{One install, one config.} Every front-end reads the same
    \texttt{opencode.json} --- your ALCF provider, the \texttt{/models} menu, and
    \texttt{ALCF\_TOKEN} work identically. (A standalone \textbf{desktop app} exists too;
    the browser UI is the simplest ``app.'')
  \end{calloutbox}
\end{frame}
...
% ---------------------------------------------------------------- warm-ups
\begin{frame}{Warm-ups: three prompts, watch the loop work}
  \begin{columns}[t, totalwidth=\textwidth]
    \begin{column}{0.31\textwidth}
      \begin{featbox}{cAgent}{\faChartLine\ 1. Plot sin(x)}
        \scriptsize \emph{``Plot sin(x) on $[0,2\pi]$ with matplotlib, save
        \texttt{sin.png}, then run it.''}
        \\[4pt]{\color{midgray}No matplotlib? It \textbf{installs it and re-runs} --- that's the loop.}
      \end{featbox}
    \end{column}
    \begin{column}{0.035\textwidth}\end{column}
    \begin{column}{0.31\textwidth}
      \begin{featbox}{cRAG}{\faVial\ 2. Sort \& test}
        \scriptsize \emph{``\texttt{sort\_ints(nums)} ascending, plus pytest for empty,
        duplicates, negatives. Run them.''}
        \\[4pt]{\color{midgray}Writes code \textbf{and} tests, then \textbf{fixes its own} failures.}
      \end{featbox}
    \end{column}
    \begin{column}{0.035\textwidth}\end{column}
    \begin{column}{0.31\textwidth}
      \begin{featbox}{accentblue}{\faAlignLeft\ 3. Count words}
        \scriptsize \emph{``Top-10 words in \texttt{sample.txt}, minus stop-words.''}
        \\[4pt]{\color{midgray}Now \textbf{read} which words it treats as stop-words --- a silent decision.}
      \end{featbox}
    \end{column}
  \end{columns}
  \vspace{10pt}
  \begin{calloutbox}[accentteal]
    \small\faCheckCircle\ All three \textbf{run green} --- you described, it built. Next:
    aim the very same habit at a \textbf{factual} claim, and watch it break.
  \end{calloutbox}
\end{frame}

% ---------------------------------------------------------------- the trap
\begin{frame}[fragile]{Then trip the trap: green code, confidently wrong}
  \begin{columns}[c, totalwidth=\textwidth]
    \begin{column}{0.5\textwidth}
      {\footnotesize\bfseries\color{accentgold}\faGhost\ Aim it at a fact}
      \begin{lstlisting}[style=shell]
Add philemon_1_6() returning the exact
text of Philemon 1:6 from memory --- no
file, no internet. Add a test that it
returns a non-empty string, and run it.
      \end{lstlisting}
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{featbox}{accentgold}{\faExclamationTriangle\ What happens}
        \scriptsize The test \textbf{passes} (the string is non-empty) --- but the verse
        is very likely \textbf{fabricated}, in fluent biblical cadence. Last week's
        hallucination, now \textbf{baked into code that runs green.}
      \end{featbox}
    \end{column}
  \end{columns}
  \vspace{6pt}
  \begin{calloutbox}[accentgold]
    \small\faBalanceScale\ \textbf{A passing test proves the code runs, not that it's
    true.} \textbf{Save it} --- prompt, the generated verse, the real verse; diff them.
    Another disclosure-appendix artifact.
  \end{calloutbox}
\end{frame}
...
% ---------------------------------------------------------------- capstone
\begin{frame}{Capstone: a Luther vector database --- RAG on a real corpus}
  \begin{center}
  \begin{tikzpicture}[font=\scriptsize, node distance=0.5cm,
      s/.style={rectangle, rounded corners=3pt, draw, thick, fill=lightgray,
                drop shadow, align=center, text width=1.5cm, minimum height=0.95cm}]
    \node[s, draw=labnavy]  (d) {\faDownload\\ \textbf{Download}\\ \scriptsize Gutenberg};
    \node[s, draw=cAgent, right=of d] (c) {\faCut\\ \textbf{Chunk}\\ \scriptsize passages};
    \node[s, draw=accentblue, right=of c] (e) {\faVectorSquare\\ \textbf{Embed}\\ \scriptsize vectors};
    \node[s, draw=cRAG, right=of e] (v) {\faDatabase\\ \textbf{Vector DB}\\ \scriptsize (Chroma)};
    \node[s, draw=cEval, right=of v] (g) {\faComments\\ \textbf{Retrieve}\\ \scriptsize + ground};
    \draw[-{Latex[length=5pt]}, thick, midgray] (d)--(c);
    \draw[-{Latex[length=5pt]}, thick, midgray] (c)--(e);
    \draw[-{Latex[length=5pt]}, thick, midgray] (e)--(v);
    \draw[-{Latex[length=5pt]}, thick, midgray] (v)--(g);
  \end{tikzpicture}
  \end{center}
  \vspace{2pt}
  \begin{columns}[t, totalwidth=\textwidth]
    \begin{column}{0.48\textwidth}
      \begin{featbox}{cRAG}{\faBookOpen\ The corpus --- public-domain Luther}
        \scriptsize The \textbf{95 Theses}, \emph{Christian Liberty}, the \emph{Large
        Catechism}, \emph{Table Talk}, \emph{Galatians}\dots\ $\sim$8 works, straight
        from Project Gutenberg.
      \end{featbox}
    \end{column}
    \begin{column}{0.04\textwidth}\end{column}
    \begin{column}{0.48\textwidth}
      \begin{featbox}{accentgold}{\faBalanceScale\ The integrity test}
        \scriptsize Ask something \textbf{not} in the corpus --- it must \textbf{refuse},
        not invent. Ask for a thesis --- the citation must be \textbf{verbatim}.
      \end{featbox}
    \end{column}
  \end{columns}
  \vspace{6pt}
  \begin{center}\small\color{labnavy}%
    RAG turns the model from a \textbf{source} (that fabricates) into a \textbf{finder}
    (that quotes \& cites) --- you build it by \emph{describing} it (\texttt{examples.md}).\end{center}
\end{frame}
