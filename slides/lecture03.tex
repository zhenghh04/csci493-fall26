...
\begin{frame}[fragile]{Step 2 --- Tell opencode about ALCF (\texttt{opencode.json})}
  \begin{lstlisting}[style=shell, title=opencode.json (in your project folder)]
{
  ...
  \begin{center}\scriptsize\color{midgray}%
    Same \texttt{baseURL} \& token as Week 2 --- wrapped in one \texttt{openai-compatible} provider block.\end{center}
\end{frame}
...
# inside opencode:
/models           # pick ALCF -> Llama 3.1 8B
      \end{lstlisting}
      {\scriptsize\color{midgray}Sanity check: \emph{``What model are you, and who serves you?''}}
    \end{column}
    \begin{column}{0.5\textwidth}
      \begin{featbox}{cRAG}{\faCheckDouble\ You now have\dots}
        \scriptsize \dots\ a coding \textbf{agent} --- it can read/write files and run
        commands --- driven by an \textbf{open model you control}, on ALCF. The Week 1
        loop, on the Week 2 engine.
      \end{featbox}
    \end{column}
  \end{columns}
  \vspace{6pt}
